...
The four-direction layout guard now includes dedicated CPU1/CPU2/CPU3 multi-meld scenarios, all-player open-meld pressure, and late-hand CPU call layout. MVP-4.4.4 keeps CPU1/CPU3 pon/chi as compact vertical three-tile columns that add side-by-side, keeps CPU2 melds compact near the top seat, and rotates only the meld tile faces. Cache bust is `mvp444-cpu-side-meld-columns-1`. Current local checks target all smartphone landscape layout scenarios, the CPU win reachability diagnostic, and the CPU call frequency diagnostic.
...
CPU pon remains higher priority than CPU chi. Kan, scoring, furiten, full multi-caller competition, and CPU difficulty UI remain out of scope. The MVP-4.0 plan stays in `docs/mvp-40-cpu-call-plan.md` as the staged design for CPU calls.
